...
Сервер определяет роль по bearer-токену и ограничивает административные операции на уровне API.
...
Для маршрутов с авторизацией:

- `401 unauthorized`: отсутствует, повреждён или истёк токен
- `403 forbidden`: токен валиден, но прав для операции недостаточно
...
1. Пользователь или администратор входит через `POST /auth/login`.
2. Клиент получает текущую роль через `GET /me`.
3. Пользователь работает с доступными ресурсами и своими бронированиями.
4. Администратор через тот же API получает доступ к управлению ресурсами.
5. Администратор контролирует общую загрузку через `GET /admin/bookings` и `GET /admin/reports/utilization`.
...
- JWT-аутентификация реализована через `github.com/golang-jwt/jwt/v5`.
- Для production-версии стоит добавить refresh-токены, ротацию секретов и более строгую политику хранения сессий.
