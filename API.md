...
- Текущая версия использует in-memory хранилище, поэтому данные сбрасываются при перезапуске сервера.
- Токен реализован без внешних библиотек, в формате JWT-подобной строки, совместимой с текущим сервером.
- Для production-версии рекомендуется заменить хранилище на `PostgreSQL` и подключить полноценную JWT-библиотеку.
