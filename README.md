...
2. Запустите сервер и консоль администратора:

```bash
go run .
```

Сервер слушает `:8080`, а в терминале открывается консольное меню администратора с навигацией по цифрам.
Логи сервера не засоряют главное меню, доступны через пункт `Логи` и обновляются на экране автоматически.
Штатного пункта выхода из админки нет: остановка сервера и CLI выполняется через `Ctrl+C`.
...
- открывать отдельный экран последних серверных логов.
